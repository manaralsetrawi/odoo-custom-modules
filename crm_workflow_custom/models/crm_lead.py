...
class CrmLead(models.Model):
    ...
    estimated_duration = fields.Char(string="Estimated Duration", tracking=True)
    ...
    proposal_amount = fields.Float(string="Proposal Amount", tracking=True)
    ...
    inactive_alert = fields.Boolean(string="Inactive Alert", default=False, tracking=True)
    ...
    #Workflow buttons methods and helper method to get stage by name
    ...
    def action_submit_proposal(self):
        for record in self:
            ...
            if not record.proposal_amount:
                raise ValidationError(_("Please enter the proposal amount before submitting the proposal."))
    ...
    def action_send_to_approval(self):
        for record in self:
            if not record.proposal_summary or not record.proposal_amount or not record.project_deadline:
                raise ValidationError(_("Proposal details, amount, and deadline must be filled before sending for approval."))
