...
class ExpenseRequest(models.Model):
    ...
    manager_approval_date = fields.Datetime(
        string='Manager Approval Date', readonly=True)

    finance_approved_by = fields.Many2one(
        'res.users', string='Finance Approved By', readonly=True)
    finance_approval_date = fields.Datetime(
        string='Finance Approval Date', readonly=True)
    ...
    def action_submit(self):
        for record in self:
            if not record.attachment:
                raise ValidationError(
                    'Please attach a receipt or invoice before submitting.')
    ...
    def action_manager_approve(self):
        for record in self:
            ...
            if not record.manager_user_id or record.manager_user_id != self.env.user:
                raise ValidationError(
                    'Only the employee manager can approve this expense request.')
    ...
    def action_manager_reject(self):
        for record in self:
            ...
            if not record.manager_user_id or record.manager_user_id != self.env.user:
                raise ValidationError(
                    'Only the employee manager can reject this expense request.')

            if not record.rejection_reason:
                raise ValidationError(
                    'Please enter a rejection reason before rejecting the expense request.')
    ...
    def action_mark_paid(self):
        ...
        for record in self:
            if record.state != 'approved_finance':
                raise ValidationError(
                    'Only finance-approved expenses can be marked as paid.')

            if not record.budget_reservation_id or record.budget_reservation_id.state != 'reserved':
                raise ValidationError(
                    'The related budget reservation must be in Reserved state before marking as paid.')
